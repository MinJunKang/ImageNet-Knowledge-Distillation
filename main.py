...
def main():
    ...
    dataset = Imagenet.Dataset(50,800)
    ...
    if(args_setup.model_usage == "teacher"):
        ...
        if(uinput == 1):
            ...
            teacher_model = Teacher(args_train,args_setup.model_usage,dataset.label_num)
            teacher_model.Train(args_train,dataset.train_dir,dataset.valid_dir,dataset.train_num,dataset.valid_num)
        ...
        # Prepare Soft Target
            ...
            result = teacher_model.Evaluate(dataset.test_dir,dataset.test_num)
            # Save accuracy
            ...
            teacher_model.Save_soft_value(dataset.train_dir,dataset.train_num,args_train.temperature)
        else:
            teacher_model = Teacher(args_train,args_setup.model_usage,dataset.label_num)
        ...
        if(uinput == 4):
            ...
            student_model.Train(args_train,dataset.train_dir,dataset.valid_dir,dataset.train_num,dataset.valid_num) ##add directory
        elif(uinput == 5):
            Header.check_and_makedir(args_train.dir)
            Header.check_and_makedir(args_train.checkpoint,True)
            Header.check_and_makedir(args_train.model,True)
            ...
            if(Header.os.path.isfile(args_train.softy_file) == False):
                print("First train the Teacher model to get soft targets")
                exit(1)         
            student_model.Train(args_train,dataset.train_dir,dataset.valid_dir,dataset.train_num,dataset.valid_num,True) ##add directory

        elif(uinput == 6):
            student_model = Student(args_train,args_setup.model_usage)
            teacher_model = Teacher(args_train,args_setup.model_usage,dataset.label_num)
            if(Header.os.path.isfile(args_train.softy_file) == False):
                result  = student_model.Evaluate(args_train,dataset.test_dir,None,dataset.test_num) ##add directory
                print("Test accuracy : %f" %(result[1]))
            else:
                [train_t_acc,valid_t_acc,test_t_acc] = student_model.Load_Text(args_train.teacher_acc_file)
                result  = student_model.Evaluate(args_train,dataset.test_dir,args_train.softy_file,dataset.test_num) ##add directory
            ...
            [acc,num_param] = student_model.Fit_SingleModel(dataset.train_dir,dataset.valid_dir,dataset.test_dir,args_train.softy_file,dataset.train_num,dataset.valid_num,dataset.test_num,hidden_node,learn_rate,batch) ##add directory
            [train_t_acc,valid_t_acc,test_t_acc] = student_model.Load_Text(args_train.teacher_acc_file)
